...
const JadwalForm = ({ mode = 'create', initialData = null, onSubmit, onCancel, loading = false }) => {
  ...
  const mockMataPelajaran = [
    { id: 1, nama: 'Matematika', kode: 'MTK' },
    { id: 2, nama: 'Bahasa Indonesia', kode: 'BIND' },
    { id: 3, nama: 'Pemrograman Web', kode: 'PWEB' },
    { id: 4, nama: 'Jaringan Komputer', kode: 'JKM' },
    { id: 5, nama: 'Database', kode: 'DB' },
  ];

  const mockGuru = [
    { id: 1, nama: 'Pak Budi Santoso', mata_pelajaran_ids: [1, 3] },
    { id: 2, nama: 'Bu Ani Wijaya', mata_pelajaran_ids: [2] },
    { id: 3, nama: 'Pak Joko Susilo', mata_pelajaran_ids: [3, 5] },
    { id: 4, nama: 'Bu Siti Nurhaliza', mata_pelajaran_ids: [4] },
  ];

  const mockKelas = [
    { id: 1, nama: 'X RPL 1', jurusan: 'RPL' },
    { id: 2, nama: 'X RPL 2', jurusan: 'RPL' },
    { id: 3, nama: 'XI TKJ 1', jurusan: 'TKJ' },
    { id: 4, nama: 'XI MM 1', jurusan: 'MM' },
  ];
  ...
  // Filter guru by selected mata pelajaran
  const filteredGuru = formData.mata_pelajaran_id
    ? mockGuru.filter(guru => 
        guru.mata_pelajaran_ids.includes(parseInt(formData.mata_pelajaran_id))
      )
    : mockGuru;

  // Check for time conflicts (mock implementation)
  useEffect(() => {
    if (formData.hari && formData.jam_mulai && formData.jam_selesai && formData.kelas_id) {
      // Mock conflict check
      // In real app, this would call API to check existing jadwal
      const hasConflict = false; // Mock: no conflict
      
      if (hasConflict) {
        setConflict({
          message: 'Jadwal bentrok dengan Matematika (07:00-08:30) di kelas yang sama',
          type: 'time'
        });
      } else {
        setConflict(null);
  ...
  }, [formData.hari, formData.jam_mulai, formData.jam_selesai, formData.kelas_id]);
